        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <div class="user-ctrl">
              ...
              <el-link type="primary" @click="borrowBook(scope.row)">预约</el-link>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    export default {
      ...
      methods: {
        async findBook() {
          ...
          const res = await this.$http.get(this.baseUrl+`/findBook2?name=${submit}`)
        ...
        async borrowBook(row) {
          this.$confirm(`确定要预约这本 ${row.author} 的 ${row.book_name} 吗？`, '提示', {
